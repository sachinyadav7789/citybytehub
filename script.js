...
const BOOKING_AUTO_ADVANCE_SKIP_IDS = new Set([
    ...
    'bk-track-phone',
    'bk-track-input',
    // Let users finish typing/selecting both time fields without focus jumps.
    'bk-time',
    'bk-end-time'
]);
...
const FORM_DRAFT_IDS = [
    ...
    // Booking identity, date and time fields intentionally stay fresh on every visit.
    // This prevents an old draft from overwriting what the user is currently typing.
    'bk-service', 'bk-card', 'bk-note',
...
let _liveBookingAvailability = { gamingPcSeats: null, ps5Consoles: null, internetBrowsingPcs: null, mobileSeats: null, openHours: '' };
...
function applyLiveSeats(value) {
    ...
    if (el) {
        ...
        el.classList.remove('availability-value--good', 'availability-value--warning', 'availability-value--danger', 'availability-value--muted');
        el.classList.add(!Number.isFinite(v) ? 'availability-value--muted' : v <= 0 ? 'availability-value--danger' : v <= 2 ? 'availability-value--warning' : 'availability-value--good');
    }
    _lastLiveSeats = v;
}
...
let _slotAvailabilityRequestId = 0;
...
function applyBookingTimeConstraints() {
    ...
    const stepSec = 60; // 1-minute precision for the native time picker.
...
function setAvailabilityTone(element, value, warningThreshold = 1) {
    if (!element) return;
    element.classList.remove('availability-value--good', 'availability-value--warning', 'availability-value--danger', 'availability-value--muted');
    if (!Number.isFinite(value)) {
        element.classList.add('availability-value--muted');
    } else if (value <= 0) {
        element.classList.add('availability-value--danger');
    } else if (value <= warningThreshold) {
        element.classList.add('availability-value--warning');
    } else {
        element.classList.add('availability-value--good');
    }
}
...
// ===== CARD PREVIEW =====
function applyPrimePlan(plan) {
    const selectedPlan = plan === 'monthly' ? 'monthly' : 'weekly';
    ...
    const weeklyCard = $('card-preview-weekly');
    const monthlyCard = $('card-preview-monthly');
    const weeklyBenefits = $('plan-benefits-weekly');
    const monthlyBenefits = $('plan-benefits-monthly');

    if (weeklyCard) weeklyCard.hidden = !isWeekly;
    if (monthlyCard) monthlyCard.hidden = isWeekly;
    if (weeklyBenefits) weeklyBenefits.hidden = !isWeekly;
    if (monthlyBenefits) monthlyBenefits.hidden = isWeekly;

    document.querySelectorAll('[data-plan]').forEach(button => {
        const buttonPlan = button.dataset.plan;
        button.classList.toggle('active', buttonPlan === selectedPlan);
    });
}

window.switchCardPreview = function (plan) {
    applyPrimePlan(plan);
};
...
window.checkCardStatus = async function () {
    ...
    if (!rateLimitCheck('cbh_card_lookup_rl', 20, 3600000)) {
        ...
        res.innerHTML = '<span class="u-js-497726e">❌ Too many checks. Please try again after 1 hour.</span>';
        return;
    }
    if (!/^CBH-[A-Z0-9-]{6,}$/.test(val)) {
        res.style.display = 'block';
        res.innerHTML = '<span class="u-js-497726e">❌ Only a valid card number can be checked (phone lookup disabled).</span>';
        return;
    }
    res.style.display = 'block';
    res.innerHTML = '<span class="u-js-57f9e31">🔍 Checking...</span>';
    try {
        let cards = [];
        const s1 = await getDocs(query(collection(db, 'primeCards'), where('cardNumber', '==', val)));
        s1.forEach(d => cards.push({ id: d.id, ...d.data() }));
        if (!cards.length) { res.innerHTML = '<span class="u-js-497726e">❌ Card not found.</span>'; return; }
        const c = cards[0];
        ...
        res.innerHTML = `<div class="u-js-c5192da">
      <div class="u-js-03ac0c4">${sl}</div>
      <div class="u-js-2eec8ea">
        <b>Card:</b> ${escHTML(maskCard(c.cardNumber || '—'))}<br>
        <b>Name:</b> ${escHTML(maskName(c.name || ''))}<br>
        <b>Plan:</b> ${c.plan === 'weekly' ? '📅 Weekly' : '📆 Monthly'}<br>
        <b>Balance:</b> <span class="u-js-e39ebac">${escHTML(c.balance || '—')}</span><br>
        <b>Valid Till:</b> ${escHTML(c.expiry || '—')}<br>
        ${c.status !== 'active' ? '<span class="u-js-1e0d123">Card is not active — please contact staff.</span>' : ''}
      </div>
    </div>`;
    } catch (e) {
        console.error('Card status lookup failed:', e);
        res.innerHTML = '<span class="u-js-497726e">❌ Status could not be fetched. Please try again later.</span>';
    }
};
...
function renderBookingLiveRateList() {
    ...
    <div class="avail-row">
      ...
      <span class="avail-val u-js-15434ad">${escHTML(r.text)}</span>
    </div>
  `;
        }
        return `
    <div class="avail-row">
      <span class="avail-label">${escHTML(r.label)}</span>
      <span class="avail-val u-js-e39ebac">Rs.${r.rate}${r.unit || '/hr'}</span>
    </div>
  `;
    }).join('');

    host.innerHTML = rowHtml || '<div class="u-js-57f9e31">Rates unavailable right now.</div>';
}
...
function syncBookingDurationFromTimeRange() {
    ...
    if (!durInput) return null;

    // Duration starts empty. It becomes read-only only after both times form
    // a valid range; the user can otherwise enter it manually if needed.
    const rangeDur = (
        isValidTimeHHMM(startTime) &&
        isValidTimeHHMM(endTime)
    ) ? durationFromTimeRange(startTime, endTime) : null;

    if (rangeDur) {
        durInput.value = String(rangeDur);
        durInput.readOnly = true;
        durInput.setAttribute('aria-readonly', 'true');
        durInput.title = 'Duration is automatically calculated from Start Time and End Time.';
...
function updateBookingBillPreview() {
    ...
    syncBookingDurationFromTimeRange();
...
function setBookingDateDefaults({ keepUserSelection = true } = {}) {
    const di = $('bk-date');
    if (!di) return;
    ...
    if (!keepUserSelection || !di.value || di.value < today || di.value > tomorrow) {
        di.value = today;
    }
...
function scheduleBookingDateRollover() {
    const run = () => {
        const di = $('bk-date');
        if (!di) return;
        ...
        if (!di.value || di.value < today || di.value > tomorrow) {
            di.value = today;
        }

        di.min = today;
        di.max = tomorrow;
        checkAdvanceBooking(di.value);
    };
    ...
    window.setTimeout(() => {
        run();
        window.setInterval(run, 24 * 60 * 60 * 1000);
...
function initBookingTimeInputs() {
    ['bk-time', 'bk-end-time'].forEach(id => {
        const input = $(id);
        if (!input) return;
        ...
        /*
         * IMPORTANT:
         * Do not touch .value on input/blur for native <input type="time">.
         * Chrome/Edge expose the time as segmented hour/minute/AM-PM controls.
         * Running validation/re-render work on every segment while the user
         * types can cause the native control to lose its partial value.
         *
         * We therefore wait for the browser's `change` event. At that point
         * the native value is complete (HH:MM) and it is safe to calculate
...
// Date limits: today is the default; tomorrow is the only other allowed date.
(function initBookingDateField() {
    const di = $('bk-date');
    if (!di) return;

    setBookingDateDefaults({ keepUserSelection: false });

    // Always start a fresh booking with blank identity/time fields.
    ['bk-name', 'bk-phone', 'bk-time', 'bk-end-time', 'bk-duration'].forEach(id => {
        const field = $(id);
        if (field) field.value = '';
    });
    const durationField = $('bk-duration');
    if (durationField) {
        durationField.readOnly = false;
        durationField.removeAttribute('aria-readonly');
    }
    ...
    di.addEventListener('change', function () {
        ...
        checkSlotAvail();
        checkAdvanceBooking(this.value);
    });

    scheduleBookingDateRollover();
})();
...
function renderSlotSuggestions(service, date, bookings) {
    ...
    if (!slots.length) {
        host.innerHTML = `<div class="u-js-3c66dd6">❌ No available slots for ${escHTML(priceServiceLabel(service))} in the selected window.</div>`;
        return;
    }

    const btns = slots.map(window => {
        const label = formatWindowLabel(window);
        return `<button type="button" class="btn btn-outline btn-sm u-js-75939b4" data-action="apply-suggested-slot" data-service="${service}" data-start-min="${window.startMin}" data-end-min="${window.endMin}">${escHTML(label)}</button>`;
    }).join('');

    host.innerHTML = `
    <div class="u-js-82dbeab">
      <div class="u-js-ff4db25">AVAILABLE ${escHTML(priceServiceLabel(service)).toUpperCase()} SLOTS</div>
      <div class="u-js-3c8d304">${btns}</div>
    </div>
  `;
}
...
function renderAlternateServiceSuggestions(selectedService, date, bookings) {
    ...
    const rows = services.map((service) => {
        ...
        const btns = slots.map(window => {
            const label = formatWindowLabel(window);
            return `<button type="button" class="btn btn-outline btn-sm u-js-5d79afb" data-action="apply-suggested-slot" data-service="${service}" data-start-min="${window.startMin}" data-end-min="${window.endMin}">${escHTML(label)}</button>`;
        }).join('');
        return `
      <div class="u-js-6dcc762">
        <div class="u-js-f7c8b3d">${escHTML(priceServiceLabel(service))}</div>
        <div class="u-js-622921d">${btns}</div>
      </div>
    `;
    }).filter(Boolean).join('');
    ...
    if (!rows) return;
    host.innerHTML = `
    <div class="u-js-2e655d5">
      <div class="u-js-6993959">ALTERNATE SERVICES AVAILABLE</div>
      ${rows}
    </div>
  `;
}

function getSelectedServiceAvailability(service) {
    const live = _liveBookingAvailability || {};
    const mapping = {
        'gaming-pc': { count: live.gamingPcSeats, unit: 'seats' },
        ps5: { count: live.ps5Consoles, unit: 'consoles' },
        internet: { count: live.internetBrowsingPcs, unit: 'PCs' },
        mobile: { count: live.mobileSeats, unit: 'seats' }
    };
    return mapping[service] || { count: null, unit: 'seats' };
}

function clearBookingSlotSuggestions() {
    ['slot-suggestions', 'alt-service-suggestions'].forEach(id => {
        const host = $(id);
        if (host) host.innerHTML = '';
    });
}

function renderSelectedServiceAvailability(service, date) {
    ...
    const label = priceServiceLabel(service);
    const availability = getSelectedServiceAvailability(service);

    if (Number.isFinite(availability.count)) {
        const isFull = availability.count <= 0;
        const message = isFull
            ? `❌ No ${availability.unit.toLowerCase()} currently available for ${escHTML(label)}.`
            : `✅ ${availability.count} ${availability.unit} currently available for ${escHTML(label)}. Enter Start Time and End Time to check the exact slot.`;
        box.innerHTML = `<div class="u-js-availability-summary${isFull ? ' is-full' : ''}">${message}</div>`;
    } else {
        box.innerHTML = `<div class="u-js-availability-summary">ℹ️ ${escHTML(label)} availability will be checked for your selected Start Time and End Time.</div>`;
    }
}

async function checkSlotAvail() {
    const requestId = ++_slotAvailabilityRequestId;
    const service = $('bk-service')?.value;
    ...
    renderSelectedServiceAvailability(service, date);

    // Do not force a suggested slot. The user can freely enter Start/End Time.
    // Once a time range is entered, check that exact window against active bookings.
    if (!startTime) return;

    if (!isValidTimeHHMM(startTime)) {
        box.innerHTML = '<div class="u-js-38c153e">⚠️ Select a valid start time.</div>';
        return;
    }

    if (!isWithinBookingHours(startTime)) {
        box.innerHTML = `<div class="u-js-38c153e">⚠️ Select a time between ${escHTML(bookingHoursLabel())}.</div>`;
        return;
    }

    // Keep the first message simple. Exact capacity is checked only
    // after the user enters both Start Time and End Time.
    if (!endTime) return;

    if (!isValidTimeHHMM(endTime)) {
        box.innerHTML = '<div class="u-js-38c153e">⚠️ Select a valid end time.</div>';
        return;
    }
    if (!isWithinBookingHours(endTime)) {
        box.innerHTML = `<div class="u-js-38c153e">⚠️ End time must be within ${escHTML(bookingHoursLabel())}.</div>`;
        return;
    }
    if (durationFromTimeRange(startTime, endTime) === null) {
        box.innerHTML = '<div class="u-js-38c153e">⚠️ End time must be after start time.</div>';
        return;
    }

    box.innerHTML = '<div class="u-js-b200112">⏳ Checking this exact time range...</div>';

    try {
        const bookings = await loadActiveBookingsForDate(date);
        if (requestId !== _slotAvailabilityRequestId) return;
        const requestedWindow = deriveBookingWindow(startTime, endTime, duration || 60);
        if (!requestedWindow) {
            box.innerHTML = '<div class="u-js-1405677">❌ Invalid time window. Check Start Time and End Time.</div>';
            return;
        }
        ...
        const isFull = left <= 0;
        const status = isFull
            ? `❌ No seats available for ${escHTML(priceServiceLabel(service))} in this time range.`
            : `✅ ${left} ${left === 1 ? 'seat' : 'seats'} available for ${escHTML(priceServiceLabel(service))} in this time range.`;

        box.innerHTML = `<div class="u-js-availability-summary${isFull ? ' is-full' : ''}">${status}</div>`;
    } catch (e) {
        if (requestId !== _slotAvailabilityRequestId) return;
        const code = String(e?.code || '');
        if (code.includes('permission-denied')) {
            box.innerHTML = '<div class="u-js-38c153e">⚠️ Exact slot availability is temporarily unavailable. Final validation happens on submit.</div>';
            return;
        }
        box.innerHTML = '<div class="u-js-1405677">⚠️ Could not check this time range. We will re-check on submit.</div>';
    }
}
...
setTimeout(() => {
    ...
    // Booking identity/date/time must always start fresh.
    if ($('bk-name')) $('bk-name').value = '';
    if ($('bk-phone')) $('bk-phone').value = '';
    setBookingDateDefaults({ keepUserSelection: false });

    syncBookingDurationFromTimeRange();
    applyPrimePlan($('selected-plan')?.value || 'weekly');
    updateCardPreview();
    updateBookingServiceDetailUi();
    updateBookingBillPreview();
    applyBookingTimeConstraints();
    ...
    // Time inputs are initialized separately so typing is never interrupted.
    initBookingTimeInputs();
...
window.submitBooking = async function () {
    ...
    if (!endTime) {
        showErr('bk-end-time-err', 'Enter an end time.');
        valid = false;
    ...
    try {
        ...
        if (bkOk) {
            bkOk.innerHTML = `<div class="u-js-6c516db">
        <div class="u-js-6fb1af3">✅ Booking Request Received!</div>
        <div class="u-js-c5bccfa">Save your booking code:</div>
        <div class="u-js-ad3effe">${trackCode}</div>
        <div class="u-js-d8562d0">Save this 8-character code to check status later</div>
        <div class="u-js-6aad6ad">${escHTML(estimatedText)}</div>
        <div class="u-js-2f5899b">Time: ${prettyStart}${prettyEnd ? ` - ${prettyEnd}` : ''}</div>
        ${isAdv ? `<div class="u-js-394a396">
          💳 Payment is required for tomorrow bookings<br>
          📞 Call: <a href="tel:+91${PAYMENT_PHONE}" class="u-js-dd86069">${PAYMENT_PHONE}</a>
        </div>`: `<div class="u-js-b5b0188">
          📞 We will call <strong class="u-js-6c45f98">${maskPhone(phone)}</strong> to confirm
        </div>`}
      </div>`;
...
window.checkBookingStatus = async function () {
    ...
    if (!/^[6-9][0-9]{9}$/.test(phone)) {
        res.style.display = 'block';
        res.innerHTML = '<div class="u-js-7b5ac34">❌ Enter a valid 10-digit booking phone (e.g., 98XXXXXXXX). +91 is OK.</div>';
        return;
    }
    if (!consumeWindowRateLimit('cbh_booking_lookup_rl_' + phone, 5, 3600000)) {
        res.style.display = 'block';
        res.innerHTML = '<div class="u-js-7b5ac34">❌ Too many checks. Please try again after 1 hour.</div>';
        return;
    }
    const lookupServerGuard = await evaluateServerAbuseGuard('bk_lookup', phone);
    if (!lookupServerGuard.ok) {
        res.style.display = 'block';
        res.innerHTML = `<div class="u-js-7b5ac34">❌ ${abuseReasonText(lookupServerGuard.reason, 'Request limit exceeded. Please try again later.')}</div>`;
        return;
    }
    const code = inp.value.trim().toUpperCase();
    if (!code || !(/^[A-Z0-9]{8}$/.test(code) || /^[0-9]{5}$/.test(code))) {
        res.style.display = 'block';
        res.innerHTML = '<div class="u-js-7b5ac34">❌ Enter a valid booking code (e.g., A1B2C3D4)</div>';
        return;
    }
    res.style.display = 'block';
    res.innerHTML = '<div class="u-js-6958676">🔍 Searching...</div>';
    try {
        ...
        if (snap.empty) {
            res.innerHTML = `<div class="u-js-0970d04">
        ❌ Booking details could not be verified.<br>
        <span class="u-js-b11141f">WhatsApp: <a href="https://wa.me/918829822950" class="u-js-15434ad">8829822950</a></span>
      </div>`;
            return;
        }
        ...
        <span class="u-js-b11141f">Hint: This code is linked with phone ${escHTML(expectedMasked)}.</span>
      </div>`;
            return;
        }
        const b = matchedDoc.data();
        const statusClass = { pending: 'status-pending', confirmed: 'status-confirmed', rejected: 'status-rejected', pending_payment: 'status-pending-payment' };
        const sl = {
        ...
        const sStatusClass = statusClass[b.status] || 'status-pending';

        res.innerHTML = `<div class="booking-status-card ${sStatusClass}">
      <div class="booking-status-heading">${sl[b.status] || '⏳ Pending'}</div>
      <div class="booking-status-details">
        <b>Customer:</b> ${escHTML(maskName(b.name || ''))}<br>
        <b>Date:</b> ${escHTML(b.date || '—')}<br>
        <b>Code:</b> <span class="booking-status-code">${escHTML(code)}</span>
      </div>
      ${b.status === 'rejected' ? `<div class="booking-status-rejected-note">
        📞 New slot: <a href="https://wa.me/918829822950" class="link-cyan-strong">WhatsApp 8829822950</a>
      </div>`: ''}
    </div>`;
    } catch (err) {
        ...
        res.innerHTML = `<div class="u-js-a2a4335">❌ ${displayMsg}</div>`;
    }
};
...
/* =========================================================
   NAVIGATION & UI EVENT CONTROLLERS
   ========================================================= */
...
const PAGE_TITLES = {
    home: 'Home', services: 'Services', pricing: 'Pricing', prime: 'Prime Card',
    booking: 'Book a Slot', inquiry: 'Contact & Inquiry', about: 'About Us', faq: 'FAQ', reviews: 'Reviews'
};

const ROUTE_KEY = 'cbh_booking_last_page_v1';
const ROUTE_TTL_MS = 7 * 60 * 1000;

function syncTopbarHeight() {
    const topbar = document.querySelector('.topbar');
    const height = topbar ? Math.ceil(topbar.getBoundingClientRect().height) : 56;
    document.documentElement.style.setProperty('--topbar-h', `${height}px`);
}

function saveLastRoute(page) {
    try { localStorage.setItem(ROUTE_KEY, JSON.stringify({ page, ts: Date.now() })); }
...
function readLastRoute() {
    try {
        ...
        if (!page || !ts || Date.now() - ts > ROUTE_TTL_MS) return '';
        return page;
    } catch (_) { return ''; }
}

function navTo(element, page, options = {}) {
    const resolvedPage = PAGE_MAP[page] ? page : 'home';
    const behavior = options.behavior || 'smooth';
    const persist = options.persist !== false;

    document.querySelectorAll('.nav-item').forEach(item => item.classList.remove('active'));
    const navElement = element || document.querySelector(`.nav-item[href="#${resolvedPage}"]`);
    navElement?.classList.add('active');

    document.querySelectorAll('.page-section').forEach(section => section.classList.remove('active'));
    const section = document.getElementById(PAGE_MAP[resolvedPage]);
    section?.classList.add('active');
    section?.querySelectorAll('.reveal').forEach(item => item.classList.add('visible'));

    const title = document.getElementById('topbar-title');
    if (title) title.textContent = PAGE_TITLES[resolvedPage] || 'CityByteHub';

    const target = document.getElementById(resolvedPage);
    const topbar = document.querySelector('.topbar');
    ...
    if (persist) saveLastRoute(resolvedPage);
...
function toggleSidebar() {
    ...
    $('menu-toggle')?.setAttribute('aria-expanded', String(sidebar?.classList.contains('open') || false));
}
...
function initUiEvents() {
    document.addEventListener('click', event => {
        ...
        if (nav) {
            ...
            navTo(nav, nav.dataset.navTarget);
            return;
        }
        ...
        if (action) {
            switch (action.dataset.action) {
                ...
                case 'dismiss-announcement': dismissAnnouncementBar(); break;
                case 'submit-prime': window.submitPrimeApplication?.(); break;
                case 'check-card-status': window.checkCardStatus?.(); break;
                case 'submit-booking': window.submitBooking?.(); break;
                case 'check-booking-status': window.checkBookingStatus?.(); break;
                case 'submit-inquiry': window.submitInquiry?.(); break;
                case 'apply-suggested-slot':
                    window.applySuggestedSlot?.(action.dataset.service, Number(action.dataset.startMin), Number(action.dataset.endMin));
                    break;
            }
        }
        ...
        if (previewButton) window.switchCardPreview?.(previewButton.dataset.cardPreview, previewButton);

        const planButton = event.target.closest('[data-form-plan]');
        if (planButton) window.setFormPlan?.(planButton.dataset.formPlan);
    });
    ...
    document.addEventListener('submit', event => {
        ...
        if (action === 'submit-prime') window.submitPrimeApplication?.();
        if (action === 'submit-booking') window.submitBooking?.();
        if (action === 'submit-inquiry') window.submitInquiry?.();
    });

    document.addEventListener('input', event => {
        if (event.target.closest('[data-input-action="update-card-preview"]')) window.updateCardPreview?.();
    });

    document.addEventListener('keydown', event => {
        if (event.key === 'Enter' && event.target.closest('[data-enter-action="check-booking-status"]')) {
            window.checkBookingStatus?.();
        }
    });
    ...
    const startPage = PAGE_MAP[fromHash] ? fromHash : (PAGE_MAP[saved] ? saved : 'home');
    navTo(null, startPage, { behavior: 'auto', persist: false });
}
...
window.addEventListener('hashchange', () => {
    ...
    if (PAGE_MAP[page]) navTo(null, page, { behavior: 'auto' });
});
